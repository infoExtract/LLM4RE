...
def create_batches(tmp_dict, data_dict, prompt_path):
    ...
    for idx, dict_ in tmp_dict.items():
        ...
        prompt = granularity_checker_prompt.replace('$TEXT$', source_text).replace('$TRIPLE$', json.dumps(triples))
...
def main(args):
    out_path = f'{args.base_path}/granularity_batches'
            ...
            for file in files:
                ...
                if check:
                    ...
                    batches = create_batches(tmp_dict, data_dict,
                                             '/home/UFAD/aswarup/research/Relation-Extraction/LLM4RE/prompts/granularity_checker.txt')
